...
int main(int argc, char * argv[]) {
    ...
    cout<<"Escriba el numero de procesos: ";
    cin>>rank;
    ...
    A = new long *[rank]; // Reservamos tantas filas como procesos haya
    x = new long [rank]; // El vector sera del mismo tamanio que el numero
    // de procesadores

    // Solo el proceso 0 ejecuta el siguiente bloque
    if (my_rank == 0) {
        A[0] = new long [rank * rank];
    ...
    MPI_Scatter(A[0], // Matriz que vamos a compartir
            rank, // Numero de columnas a compartir
    ...
    MPI_Bcast(x, // Dato a compartir
            rank, // Numero de elementos que se van a enviar y recibir
