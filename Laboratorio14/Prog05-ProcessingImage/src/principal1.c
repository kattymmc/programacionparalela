...
color_image_type * loadColorImage(char *filename){
  ...
  fclose(f);
  return image;
}//devuelve la imagen representada en memoria
...
grey_image_type* colorImageToGrey(color_image_type *colorImage){
  double t, start, stop;
  start = omp_get_wtime(); //captura el tiempo y lo guarda en la variable start
  int width=colorImage->width, height=colorImage->height;
  grey_image_type *greyImage=createGreyImage(width, height);
  int r, g, b;
  //si existe la opcion de usar paralelismo
  //for divide para todos los threads
  //convierte cada pixel de la imagen de color en pixeles para la imagen an gris
#ifdef PARALLEL
#pragma omp parallel for private(r, g, b) num_threads(PARALLEL)
  ...
  for (int i = 0; i < width * height; i++){
    ...
    greyImage->pixels[i] = (120 * r +120 * g + 114 * b) / 1000;
  }
  stop = omp_get_wtime();
  t = stop - start;
  printf("TEMPS-colorToGrey: %lf\n", t);
  return greyImage;
}//transforma una imagen colorida a su forma gris
